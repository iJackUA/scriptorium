...
**Agent and Model.** A default Agent and Model live in workspace settings, so the user is not choosing every time, and both can be overridden per Book — spending more on a book they care about and less on one they don't. Two Models are used by task: a cheap one for mechanical work (metadata inference, Term extraction) and a stronger one for translation, where quality is the entire point.
...
- [ ] Workspace settings hold a default Agent, a mechanical Model, and a translation Model
- [ ] `book.toml` can override Agent and Model, and the override wins
- [ ] The Agent field accepts exactly one legal value in v1 and rejects others, while the file format stays stable for a second Agent
